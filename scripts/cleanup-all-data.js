...
async function clearStorageBucket(bucketName) {
  ...
  try {
    ...
    if (deleteError) {
      console.warn(`⚠️  Some files in ${bucketName} could not be deleted:`, deleteError.message)
    } else {
    ...
    if (!folderError && folders && folders.length > 0) {
      ...
      console.log(`✅ Cleared ${folders.length} files from ${bucketName}/products`)
    }
...
async function main() {
  ...
  console.log('⚠️  This will delete ALL data from public tables and storage buckets!')
