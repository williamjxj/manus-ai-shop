...
export function useProductCRUD({
  ...
  const validateProduct = useCallback(
    (data: any, isUpdate: boolean = false): ValidationResult => {
    ...
    async (
      ...
      try {
        ...
        if (newFilters.price_min !== undefined)
          params.append('price_min', newFilters.price_min.toString())
        if (newFilters.price_max !== undefined)
          params.append('price_max', newFilters.price_max.toString())
        if (newFilters.rating_min !== undefined)
          params.append('rating_min', newFilters.rating_min.toString())
        ...
        if (newFilters.collection_id)
          params.append('collection_id', newFilters.collection_id)
  ...
  const createProduct = useCallback(
    async (data: any): Promise<Product | null> => {
  ...
  const updateProduct = useCallback(
    async (id: string, data: any): Promise<Product | null> => {
  ...
  const bulkArchive = useCallback(
    async (productIds: string[], archived: boolean): Promise<boolean> => {
      setUpdating(true)
      try {
        const promises = productIds.map((id) =>
        ...
        if (successful > 0) {
          toast.success(
            `${successful} products ${archived ? 'archived' : 'unarchived'} successfully!`
          )
        }

        if (failed > 0) {
          toast.error(
            `Failed to ${archived ? 'archive' : 'unarchive'} ${failed} products`
          )
        }
  ...
  const bulkFeature = useCallback(
    async (productIds: string[], featured: boolean): Promise<boolean> => {
      setUpdating(true)
      try {
        const promises = productIds.map((id) =>
          fetch(`/api/products/${id}`, {
        ...
        if (successful > 0) {
          toast.success(
            `${successful} products ${featured ? 'featured' : 'unfeatured'} successfully!`
          )
        }

        if (failed > 0) {
          toast.error(
            `Failed to ${featured ? 'feature' : 'unfeature'} ${failed} products`
          )
        }
